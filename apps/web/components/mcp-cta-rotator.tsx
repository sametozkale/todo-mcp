...
export function McpCtaRotator({ userId }: Props) {
  ...
  return (
    <div className="pointer-events-none fixed bottom-6 right-4 z-20 sm:right-6 lg:right-12">
      <Button
        type="button"
        size="sm"
        variant="secondary"
        onPress={() => router.push(`/integrations?platform=${current.platform}`)}
        className="pointer-events-auto h-[30px] min-h-[30px] max-w-[min(calc(100vw-2rem),14rem)] gap-1.5 rounded-2xl border border-[#eaeaea] !bg-white px-2 text-left text-[11px] font-medium text-foreground shadow-[0_2px_12px_-4px_rgba(0,0,0,0.08)]"
      >
